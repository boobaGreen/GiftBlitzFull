...
*   Generazione chiavi crittografiche (AES) lato client per criptare il codice della card.

### Phase 3: Core User Flows + NFT Minting
*   **Seller Flow:**
    *   Input dati card → Encrypt (client-side).
    *   Call SC `createBox` (Deposit Trust Deposit = 100% del prezzo).
    *   Se primo trade → Minta Soulbound NFT automaticamente.
    *   Salvataggio metadata off-chain.
*   **Buyer Flow:**
...
*   **Reveal Flow:**
    ...
    *   *Nota:* Il design atomico richiede che la chiave venga rivelata on-chain o via swap atomico. Per MVP semplice: Seller invia chiave off-chain dopo lock, se non lo fa → Burn.
*   **Finalize Flow:**
