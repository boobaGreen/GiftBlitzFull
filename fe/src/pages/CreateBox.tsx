...
const CreateBox: React.FC = () => {
    ...
    const { addBox, user, repNftId, mintProfile: marketMintProfile } = useMarket();
    ...
    const handleMintProfile = async () => {
        setIsMinting(true);
        try {
            await marketMintProfile();
            showToast("Identity Created! You can now list your gift cards.", "success");
        } catch (err) {
            console.error("Minting failed:", err);
            showToast(err instanceof Error ? err.message : "Failed to create identity", "error");
        } finally {
            setIsMinting(false);
        }
    };
        ...
        <div className="relative min-h-screen pb-24 overflow-hidden">
            ...
            <div className="max-w-2xl mx-auto space-y-12">
                ...
                <div className="text-center space-y-4 px-4">
                    ...
                    <h1 className="text-5xl font-black text-white tracking-tighter">SELL <span className="text-cyan-400">ASSET</span></h1>
                    <p className="text-gray-500 font-medium max-w-md mx-auto">Lock your assets into immutable smart contracts for secure trustless trading.</p>
                </div>
                ...
                <AnimatePresence mode="wait">
                    {!repNftId ? (
                        <motion.div
                            ...
                            className="relative group p-1 rounded-[3rem] bg-gradient-to-br from-cyan-500/10 to-transparent shadow-2xl mx-4"
                        >
                            <div className="bg-slate-950/40 backdrop-blur-3xl rounded-[2.9rem] p-12 border border-white/5 text-center space-y-8">
                                <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-cyan-500/20 to-purple-500/20 border border-white/10 flex items-center justify-center mx-auto shadow-2xl">
                                    <Shield className="w-12 h-12 text-cyan-400 animate-pulse" />
                                </div>
                                <div className="space-y-3">
                                    <h2 className="text-3xl font-black text-white tracking-tight">Setup Your Passport</h2>
                                    <p className="text-gray-400 font-medium leading-relaxed max-w-sm mx-auto">
                                        To list assets on our network, you need a <span className="text-cyan-400">Citizen Passport</span>. 
                                        This enables the Mirror Protocol to enforce trade safety.
                                    </p>
                                </div>

                                <div className="p-6 rounded-[2rem] bg-cyan-500/5 border border-cyan-500/10 space-y-3 text-left">
                                    <div className="flex items-center gap-3 text-cyan-400 font-black text-[10px] uppercase tracking-widest">
                                        <Zap className="w-4 h-4 fill-current" />
                                        Passport Benefits
                                    </div>
                                    <ul className="text-[11px] text-gray-500 space-y-2 font-medium">
                                        <li>• Unlock trust-based listing capacity up to €1000</li>
                                        <li>• Secure your trades with an On-Chain Security Vault</li>
                                        <li>• Enable cross-device Cloud Recovery for your assets</li>
                                    </ul>
                                </div>
                                ...
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.98 }}
                                    className="w-full py-6 rounded-[2rem] bg-white text-black font-black uppercase tracking-[0.3em] transition-all shadow-[0_0_40px_rgba(255,255,255,0.1)] hover:shadow-[0_0_40px_rgba(255,255,255,0.2)] flex items-center justify-center gap-3"
                                >
                                    {isMinting ? (
                        ...
                        <motion.div
                            ...
                            initial={{ opacity: 0, scale: 0.95, y: 30 }}
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            className="relative group p-1 rounded-[3rem] bg-gradient-to-br from-white/10 to-transparent shadow-2xl mx-4"
                        >
